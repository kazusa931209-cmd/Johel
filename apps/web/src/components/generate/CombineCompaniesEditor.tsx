...
import { CombineCompanyDialog } from "@/components/generate/CombineCompanyDialog";
import {
  formatCompanyPeriod,
...
type CombineCompaniesEditorProps = {
...
export function CombineCompaniesEditor({
  companies,
  onChange,
  error,
  onClearError,
}: CombineCompaniesEditorProps) {
  const t = useT();
  ...
  function saveEntry(entry: CombineCompanyEntry) {
  ...
  function experienceSummary(entry: CombineCompanyEntry): string {
  ...
  const dialogInitial: CombineCompanyEntry =
      ...
      <div className="flex items-center justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-muted">
            {t("generate.combine.companiesHint")}
          </p>
        </div>
