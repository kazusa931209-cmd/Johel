...
export function resolveCombineExperiencesPerCompanyRange(maxPerCompany: number): {
  minPerCompany: number;
  maxPerCompany: number;
  thinOverlapMaxPerCompany: number;
} {
  const max = normalizeCombineExperiencesPerCompanyMax(maxPerCompany);
  const minPerCompany = max >= 2 ? 2 : 1;
  const thinOverlapMaxPerCompany = Math.min(2, max);
  return { minPerCompany, maxPerCompany: max, thinOverlapMaxPerCompany };
}
