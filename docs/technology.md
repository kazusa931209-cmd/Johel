...
- Prisma `GenerationProcess` → table `generationProcess` (one per user): `id`, `userId` (unique), `doVerdict`, `doEvaluate`, `resumeLanguage`, `downloadFormat`, `experienceAdvisePoolDepth`, `combineExperiencesPerCompanyMax`, `combineExperiencesPerCompanyMin`, `experienceDimensionMode`, `experienceJdTierDecayPercent`, `createdAt`, `updatedAt`; defaults `doVerdict`/`doEvaluate` true, `resumeLanguage` `en`, `downloadFormat` `docx`, `experienceAdvisePoolDepth` `normal`, `combineExperiencesPerCompanyMax` `5`, `combineExperiencesPerCompanyMin` `2`, `experienceDimensionMode` `technical_facet`, `experienceJdTierDecayPercent` `80`
...
- **Migrations:** under `prisma/migrations/`. Local dev (create + apply): `pnpm db:migrate`. Apply pending only: `pnpm db:migrate-deploy` (`prisma migrate deploy`). **`pnpm build`** ([`scripts/build-web.ts`](../scripts/build-web.ts)) runs `prisma generate` → `prisma migrate deploy` → `next build` — same path on CI (ephemeral `file:` SQLite), Docker entrypoint, and **Vercel Production** (Turso `libsql://` + `TURSO_AUTH_TOKEN`). The repo does **not** ship Turso import/export or custom migration-apply scripts; production schema changes land only through committed Prisma migrations at deploy time. **database is locked** on local SQLite usually means another process holds `prisma/dev.db`. Fresh Docker volume: `docker compose down -v` then `up` wipes `johel-data`.
...
- **Header token metrics** (`StudioHeaderTokenMetrics` in `StudioHeader`): horizontal **Today** / **Total** cards immediately left of **AI Assistant**; counts from `AiUsageProvider` / `GET /ai-usage/summary` (`formatTokenUsed`)
...
- `GET /settings/process` → `{ doVerdict, doEvaluate, resumeLanguage, downloadFormat, experienceAdvisePoolDepth, combineExperiencesPerCompanyMax, combineExperiencesPerCompanyMin, experienceDimensionMode, experienceJdTierDecayPercent }` (defaults: Verdict/Evaluate true, `resumeLanguage` `en`, `downloadFormat` `docx`, `experienceAdvisePoolDepth` `normal`, `combineExperiencesPerCompanyMax` `5`, `combineExperiencesPerCompanyMin` `2`, `experienceDimensionMode` `technical_facet`, `experienceJdTierDecayPercent` `80`)
- `PUT /settings/process` → same fields; `resumeLanguage` is one of `en`, `ja`, `zh-TW`, `zh-CN`, `ko`; `downloadFormat` is `docx` or `pdf` (coerced to `docx` when `resumeLanguage` is not `en`); `combineExperiencesPerCompanyMax` and `combineExperiencesPerCompanyMin` are integers `1`–`10` (min clamped to max on save); `experienceDimensionMode` is one of `star_axis`, `jd_signal`, `technical_facet`, `problem_item`; `experienceJdTierDecayPercent` is one of `30`, `50`, `70`, `80`; upsert by `userId`; returns saved values
...
- Generate / Evaluate / History **Download** uses `ResumeDownloadDropdown` + `useResumeDownload` (`downloadAs("docx" | "pdf")`, `downloadAsZip()`); **PDF** and **ZIP** disabled when resume language is not `en` via `isPdfDownloadAvailable`
...
- `POST /ai-experience-split` — body `{ experienceId }`; AI proposes `create_experience` operations only; returns `{ result, workspaceFingerprint, tokenUsed }`; `generateType: experienceSplit` (`gpt-6-luna`).
...
- Sticky workspace toolbar (JD-Resume Builder / Resume Builder): **New** (plus) + `GenerateTimeline` + `GenerateStepNavRunButton`; no duplicate page title in the toolbar (title lives in `StudioHeader`)
...
- Generate: `GenerateGenerateStep` / `EditableResumePanel` (right) + left-panel **Refine** tab (`DraftResumeRefinePanel` via `useGenerateDraftResumeSession`) — UI label **Draft resume**; left tabs on Generate: JD with Verdict → Verdict | Combine | Refine; otherwise Combine | Refine; draft JSON is stored on the generation row (`resumeJson`) when AI generates (`POST /ai-resume`, `POST /ai-general-resume`), when the user **Save**s after **Edit**, and when draft refine completes (`POST /ai-draft-refine`); **Edit** (primary, panel footer) / **Preview** + **Save** in edit mode; header Copy only (`GeneratePanelHeaderActions`); `DraftResumeRefinePanel` calls `POST /ai-draft-refine` with `generationId` (resume loaded server-side) — instruction mode (required instruction, max 10,000 chars) or experiences mode (optional workspace experience ids, optional company scene, optional instruction; at least one experience or company required); Resume Builder uses live Settings `refinePrompt` + `refineExtension`; JD-Resume Builder uses live `generatePrompt` + `generateExtension`; refine does not mutate PCE or Combine; full-screen overlay while refining; **Run** (when `doEvaluate`) navigates to Evaluate and runs `POST /ai-evaluate` (confirm when evaluation exists; reuses when `evaluationInputKey` matches, which includes `resumeHash`); **Download** on this step when `doEvaluate` is false
...
- In-progress Generate run **content** (timeline step, Job, Combine, resume, evaluation) is persisted on the **`generations`** row via `PUT` when the user leaves the JD-Resume Builder route or closes/refreshes the tab (`usePersistSnapshotOnLeave` + `fetch` `keepalive` on `pagehide`); not on every debounced session edit; **`users.currentGenerationId`** is the account-wide “current generation” pointer (updated on `POST /generations/start`, `PUT /generations/:id`, and `POST /generations/:publicId/resume`); per-device **`sessionStorage`** (`johel:generate-session-local:{userId}`) holds only client cache fields: `verdictInputKey`, `generationInputKey`, `evaluationInputKey`, `resumeAiSnapshot`, `jobDuplicateDismissedHash` (scoped by generation id); legacy full-session keys migrate once on load
- **General Resume** (`/resume-builder`): `generations.kind` = `generalResume` (JD runs = `jdResume`); public IDs **`GEN-YYYYMMDD-###`** vs JD **`JDR-YYYYMMDD-###`**; `users.currentGeneralGenerationId` + `POST/PUT/GET /general-generations/*` (Evaluate always on; snapshots omit Job); `generations.evaluationHistoryJson` stores stacked General Evaluate chat entries (`userPrompt`, `markdown`, `createdAt`); combine JSON includes per-run `platform` and `userInstruction` (not in combine defaults); `GET /general-generations/platform-suggestions` returns distinct platform labels from the user’s General Resume history for Combine autocomplete; the client loads it once per session via `loadGeneralResumePlatformSuggestions` in `cached-settings.ts` (dedupes in-flight/Strict Mode remounts). AI: `POST /ai-general-combine-recommend` (`generateType: generalCombineRecommend`); `POST /ai-general-resume` uses snapshotted `generatePrompt` on the general row; `POST /ai-general-evaluate` requires non-empty `userPrompt`; minimal execution-rules system prompt only (no snapshotted General Evaluate prompt on the API path); resume Markdown in user message; response language from prompt / explicit override; client `uiLocale` is `en`|`ko`. Not JD `/ai-resume` / `/ai-evaluate`. Defaults: `DEFAULT_GENERAL_GENERATE_PROMPT` / `DEFAULT_GENERAL_EVALUATE_PROMPT` in `@johel/prompt-defaults`; Settings **Prompts** tab **General Evaluate** (`generalEvaluatePrompt` / `generalEvaluateExtension` on `prompts`)—**Run** on Generate may prefill the Evaluate **User prompt** from live Settings prompt + extension (no API call). Combine **Run** → **Generate** runs general resume generation; **Generate** uses `GenerateGenerateStep` / `EditableResumePanel` + **Refine** (Settings Refine Prompt); **Run** on Generate navigates to **Evaluate** with optional confirms to clear evaluation history and prefill User prompt. **Evaluate**: left = editable resume (no Refine/Combine tabs); right = `GeneralResumeEvaluateStep` (required user prompt for footer **Evaluate**, chat-style history); header nav **Download** on Evaluate. Client session: `useGeneralResumeSession` + `general-resume-persistence.ts` + `general-resume-input-keys.ts`
...
- Each `aiUsage` row stores `modelName` and `generateType` via `recordAiUsage` (`src/server/lib/record-ai-usage.ts`): active `generateType` values are `verdict`, `jdMeta`, `generate`, `draftRefine` (UI label **Draft refine**), `evaluate`, `workflowRecommend`, `markdownFormat`, `authorAdvise` (UI label **Quick Experience**), `experienceAdvise`, `experienceSplit`, `combineRecommend`, `generalCombineRecommend` (UI label **General Resume Combine Recommend**), and `checkOnExperiences`; historical rows may still have `promptHelper` (label **Prompt Helper** in AI Usage History) or legacy `checkGaps` (label **Check on Experiences**); `modelName` is `auto` for Cursor, `gpt-6-astra` for OpenAI evaluate, `gpt-6-luna` for OpenAI verdict/jdMeta/workflow-recommend/author-advise/experience-advise/experience-split/combine-recommend/general-combine-recommend/check-on-experiences/draft-refine, `gpt-6-sol` for OpenAI markdown-format, `gpt-6-terra` for OpenAI resume generation
- Provider adapter under `src/server/lib/ai-verdict/`; Cursor via `@cursor/sdk` `Agent.prompt` (model `auto`, local `cwd`); OpenAI via `openai` SDK Responses API (`gpt-6-luna`, reasoning `low`)
...
- `GET /prompts` → `{ verdictPrompt, generatePrompt, evaluatePrompt, refinePrompt, generalEvaluatePrompt, verdictExtension, generateExtension, evaluateExtension, refineExtension, generalEvaluateExtension }` — empty strings when no row yet (owner only); new sign-ups receive defaults from `@johel/prompt-defaults` via `POST /auth/register` (includes `DEFAULT_REFINE_PROMPT` and `DEFAULT_GENERAL_EVALUATE_PROMPT`)
- `PUT /prompts/verdict` → body `{ verdictPrompt?, verdictExtension? }`; `PUT /prompts/generate` → `{ generatePrompt?, generateExtension? }`; `PUT /prompts/evaluate` → `{ evaluatePrompt?, evaluateExtension? }`; `PUT /prompts/refine` → `{ refinePrompt?, refineExtension? }`; `PUT /prompts/generalEvaluate` → `{ generalEvaluatePrompt?, generalEvaluateExtension? }` (system prompt trim, min 1 when provided, no max length; extension optional, max 10,000 chars; at least one field required); upsert by `userId`; on write, only the submitted system prompt is converted to markdown via AI when changed (unchanged skip conversion); requires Settings provider/apiKey when conversion runs; each endpoint returns all prompts and extensions
- Web route `/settings/prompts`: **Verdict**, **Generate**, **Evaluate**, and **Refine** tabs (`?tab=verdict|generate|evaluate|refine`, default Verdict); page copy states that system prompt changes directly affect resume generation quality; each tab shows two sections — system prompt (read-only `AiVerdictMarkdown` preview, muted placeholder when empty) and optional extension (textarea); **Edit** (pencil) opens `PromptEditDialog` (textarea + left-aligned system-impact notice, **Reset** to `@johel/prompt-defaults` draft, **Apply** — local until that tab’s **Save**); auto-markdown notice and resume-context hints where applicable; fullscreen `BusyOverlay` when conversion runs; per-tab **Save** (always enabled; inline validation on submit); toast on API result; refreshes header Token Used after save
...
- Verdict Prompt consumed by `POST /ai-verdict`; Generate Prompt consumed by `POST /ai-resume` and JD draft refine (`POST /ai-draft-refine` with `builderKind: jd`); Refine Prompt consumed by Resume Builder draft refine (`POST /ai-draft-refine` with `builderKind: general`) — when stored `refinePrompt` is empty (e.g. pre-migration accounts), `DEFAULT_REFINE_PROMPT` from `@johel/prompt-defaults` is used at request time; Evaluate Prompt consumed by `POST /ai-evaluate`
- `POST /ai-draft-refine` — shared JD and General Resume Builder Generate-step draft refine; body `{ builderKind, generationId, language, mode, instruction?, experienceIds?, companyId? }` (draft resume read from and written to `generations.resumeJson`); module `src/server/lib/ai-draft-refine/`; returns `{ resume, tokenUsed }`
...
- Module: `src/server/lib/ai-markdown-format/` — `formatMarkdownOnSave` helper; `formatExperienceFieldsOnSave` batches `problem` / `actions` / `outcome` into one AI call (JSON response, then per-field finalize); kinds `verdict` | `generate` | `evaluate` | `refine` | `companyWhatItIs` | `experienceProblem` | `experienceActions` | `experienceOutcome`
...
- Cursor via `@cursor/sdk` `Agent.prompt` (model `auto`); OpenAI via `runOpenAiMarkdownFormatResponse` (`gpt-6-sol`, reasoning `low`); usage stored as `generateType: "markdownFormat"`; AI Usage History label **Markdown Format**
...
- Provider adapter under `src/server/lib/ai-evaluate/`; Cursor via `@cursor/sdk` `Agent.prompt` (model `auto`, local `cwd`); OpenAI via `openai` SDK Responses API (`gpt-6-astra`, reasoning `high`; shared with General Resume evaluate via `runOpenAiEvaluateResponse`)
...
- `buildResumeExportBaseName` — same stem without extension (ZIP archive name `{base}.zip`, inner folder `{base}/`)
- `buildResumeProfileBundleFileName(profileDisplayName, format)` — `{Profile name}.{docx|pdf}` for files inside the ZIP folder (`resume.header.name`, sanitized)
...
- `POST /resume/zip` — same body; returns `.zip` attachment containing folder `{export base name}/` with `{resume.header.name}.docx` and `{resume.header.name}.pdf`; English-only (same constraint as PDF)
...
- CD: Vercel **Production** only (`main`); disable Preview deployments in Vercel Git settings; pending migrations applied during production build via `prisma migrate deploy` in [`scripts/build-web.ts`](../scripts/build-web.ts)
- Operator runbook: [`ci-cd.md`](./ci-cd.md) (branch protection, rollback, local `pnpm test` parity)

## Vercel + Turso (Phase 93 — production)

- **Deploy:** Vercel **Root Directory** `.` + [`vercel.json`](../vercel.json); `pnpm build` runs `prisma migrate deploy`, then `next build`
- **Env checklist (Production):** [`vercel-deploy.md`](./vercel-deploy.md)
- **Plaintext API keys:** `pnpm db:migrate-keys` once after cutover if needed (operator-run; not part of Vercel build)
...
- **Snapshot:** `PUT` on leave / tab close / refresh (same as JD-Resume Builder); explicit `saveSnapshot` before Suggest, New, download finalize, etc.; `finalized: true` when user downloads resume DOCX
- **+ New:** `PUT` current run, clear session, `POST /generations/start` for fresh ID

### Web

- Generate page: session stores `generationId` / `generationPublicId` (ID not shown in studio top bar)
...
- Studio header: left-aligned page title; right side AI assistant + account menu; logo in sidebar only
