...
- Generate: `GenerateGenerateStep` / `EditableResumePanel` (right) + left-panel **Refine** tab (`DraftResumeRefinePanel` via `useGenerateDraftResumeSession`) — UI label **Draft resume**; left tabs on Generate: JD with Verdict → Verdict | Combine | Refine; otherwise Combine | Refine; **Edit** / **Preview** toggle on Markdown derived from stored JSON; debounced `markdownToResume` commits valid edits to session `resume`; inline parse error keeps last valid JSON; panel header uses `GeneratePanelHeaderActions` (Copy + **Undo** / **Redo** over session-only draft history); `DraftResumeRefinePanel` calls shared `POST /ai-draft-refine` (`generateType: draftRefine`) with current draft JSON + resume language — instruction mode (required instruction, max 10,000 chars) or experiences mode (1–10 workspace experience ids, optional company scene, optional instruction); Resume Builder uses live Settings `refinePrompt` + `refineExtension`; JD-Resume Builder uses live `generatePrompt` + `generateExtension`; refine does not mutate PCE or Combine; full-screen overlay while refining; **Run** (when `doEvaluate`) navigates to Evaluate and runs `POST /ai-evaluate` (confirm when evaluation exists; reuses when `evaluationInputKey` matches, which includes `resumeHash`); **Download** on this step when `doEvaluate` is false
...
- **General Resume** (`/resume-builder`): `generations.kind` = `generalResume` (JD runs = `jdResume`); public IDs **`GEN-YYYYMMDD-###`** vs JD **`JDR-YYYYMMDD-###`**; `users.currentGeneralGenerationId` + `POST/PUT/GET /general-generations/*` (Evaluate always on; snapshots omit Job); combine JSON includes per-run `platform` and `userInstruction` (not in combine defaults); `GET /general-generations/platform-suggestions` returns distinct platform labels from the user’s General Resume history for Combine autocomplete; the client loads it once per session via `loadGeneralResumePlatformSuggestions` in `cached-settings.ts` (dedupes in-flight/Strict Mode remounts). AI: `POST /ai-general-combine-recommend` (`generateType: generalCombineRecommend`); `POST /ai-general-resume` and `POST /ai-general-evaluate` use snapshotted `generatePrompt` / `evaluatePrompt` on the general `generations` row (not Settings JD prompts) and PCE + user instruction + platform — not JD `/ai-resume` / `/ai-evaluate`. Default system prompts: `DEFAULT_GENERAL_GENERATE_PROMPT` / `DEFAULT_GENERAL_EVALUATE_PROMPT` in `@johel/prompt-defaults` (snapshotted on general generation start). Combine **Run** → **Generate** runs general resume generation; **Generate** uses shared `GenerateGenerateStep` / `EditableResumePanel`; **Run** on Generate → **Evaluate** runs general evaluation. Client session: `useGeneralResumeSession` + `general-resume-persistence.ts` + `general-resume-input-keys.ts`
...
- `GET /prompts` → `{ verdictPrompt, generatePrompt, evaluatePrompt, refinePrompt, verdictExtension, generateExtension, evaluateExtension, refineExtension }` — empty strings when no row yet (owner only); new sign-ups receive defaults from `@johel/prompt-defaults` via `POST /auth/register` (includes `DEFAULT_REFINE_PROMPT` for draft refine on Resume Builder Generate)
- `PUT /prompts/verdict` → body `{ verdictPrompt?, verdictExtension? }`; `PUT /prompts/generate` → `{ generatePrompt?, generateExtension? }`; `PUT /prompts/evaluate` → `{ evaluatePrompt?, evaluateExtension? }`; `PUT /prompts/refine` → `{ refinePrompt?, refineExtension? }` (system prompt trim, min 1 when provided, no max length; extension optional, max 10,000 chars; at least one field required); upsert by `userId`; on write, only the submitted system prompt is converted to markdown via AI when changed (unchanged skip conversion); requires Settings provider/apiKey when conversion runs; each endpoint returns all prompts and extensions
...
- Verdict Prompt consumed by `POST /ai-verdict`; Generate Prompt consumed by `POST /ai-resume` and JD draft refine (`POST /ai-draft-refine` with `builderKind: jd`); Refine Prompt consumed by Resume Builder draft refine (`POST /ai-draft-refine` with `builderKind: general`); Evaluate Prompt consumed by `POST /ai-evaluate`
- `POST /ai-draft-refine` — shared JD and General Resume Builder Generate-step draft refine; body `{ builderKind, generationId?, resume, language, mode, instruction?, experienceIds?, companyId? }`; module `src/server/lib/ai-draft-refine/`; returns `{ resume, tokenUsed }`
