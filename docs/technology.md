...
- Job UI (Manual): Job text max 10,000 chars + right **Run** only; URL and File tabs show an info alert (“not implemented yet / coming soon”)
...
- **PCE bundle (Phase 71, renamed Phase 72):** `GET /pce` returns `{ profiles, companies, experiences }` (unpaginated, same detail shapes as CRUD list items). Web `loadPce` / `usePce` cache one in-flight request and reuse data across Generate prerequisites, `CombineProfilePicker`, `CombineCompanyCards`, `GenerateCombineSummary`, and `CombineExperienceSuggest`. **PCE** = Profile, Company, Experience (legacy **PCEW** / “workspace” naming removed). `usePce` subscribes to PCE invalidation and refetches when `invalidateWorkspaceCrudCaches` / `clearPceCache` runs (including cross-tab via `localStorage` signal).
...
- Combine step: all workspace companies as a single-column card list; when no companies are included, cards sort by **displayPriority** (1-based); when at least one is included, included cards follow **selection order** (`combine.companies` array order) and unselected cards follow **displayPriority** via `orderCompaniesForCombineDisplay`; company selection disabled until a profile is selected; per-card include toggle (only included entries in `combine.companies`), `ViewButton` → `CompanyDetailDialog`, dual-thumb `CombinePeriodSlider` (profile `graduationYear` + `graduationMonth` through current month; end at max = `Present`), inline role context and optional **Keyword context**; Combine **Profile** (`PceSection`), **Companies**, and **Experiences** share `COMBINE_SECTION_CLASS` bordered panels; suggestion preview rows (period, role context, keyword context, suggested cards) use horizontal label + value layout; period labels in **Companies** and **Experiences** use `CombinePeriodDisplay` (`combine-period-display.ts`) — range in `text-foreground`, short inclusive duration in `text-muted` (e.g. `Jan 2025 – Present` `(1y 9m)`); suggested cards listed only in **Experiences** (`CombineSuggestedExperienceList` → `ExperienceDetailDialog` on row/**View**); company cards omit linked-experience lists; **Add Experiences** section (`CombineExperienceSuggest`) calls `POST /ai-combine-recommend` per-company hybrid (keyword context or Auto); fullscreen `BusyOverlay` while suggesting; suggestions apply immediately and show selected profile, each included company (name, period, role context, keyword context), suggested cards, rationale, and warnings; **Suggest experiences** becomes **Suggest Again** after the first run; step-bar **Run** uses `isCombineRunReady` (`runDisabled` on `GenerateStepNav`) until profile, companies, required fields, and at least one linked experience are ready
...
- **Combine context input performance:** Role context / Keyword context use local state in `CombineCompanyContextFields` and debounce (~250ms) syncing into the session snapshot so keystrokes do not write session/local storage or re-render the Generate page on every character. Pending context is flushed on blur and before Suggest / Run. `CombineCompanyCard`, period block, profile picker, and suggest section stay isolated from in-progress typing. Combine defaults (`localStorage`) persistence is debounced (~300ms).
