...
- Header **AI Assistant** (`AiAssistantTrigger` + `AiAssistantProvider` in `apps/web/src/components/app/AiAssistant.tsx`): opens categorized assistance panel on click or **⌘K** / **Ctrl+K**; v1 ships **Check the Gaps** only (`runAiCheckGaps` → `POST /ai-check-gaps`). Evaluate step text selection uses `TextSelectionToolbar` — **Check** calls `openAiAssistant({ query, category: "check-gaps", generationId })`.
- `POST /ai-check-gaps` — body `{ gapQuery, generationId? }`; loads experience pool, embeds/ranks candidates (same embedding path as experience advise), optionally loads Combine-linked experience ids from the owned generation; LLM returns structured verdict (`gap_confirmed` | `exists_not_linked` | `exists_and_linked`) formatted as Markdown; `generateType: checkGaps` (+ `embedding` for query vector)
...
- In-progress Generate run persisted in `sessionStorage` per user (`johel:generate-session:{userId}`): active timeline step, Job state, Combine snapshot (including Run guidance / `emphasis`), `verdictInputKey`, `resume` JSON, `resumeAiSnapshot` (last AI output for Revert), `generationInputKey` fingerprint (job + combine + server combine fingerprint), `evaluationMarkdown`, and `evaluationInputKey` (includes `resumeHash` via `hashResumeForCache`); legacy `oneTimePrompt` session keys migrate into `combine.emphasis` on load; **editing Job/Combine or Quick Add Experience does not clear cached results until the user Run**s from an earlier step (confirm dialog when resume/evaluation would be discarded); **resume Markdown edits** clear cached evaluation immediately via `updateResume`; `clearDownstreamFromVerdict` / `clearDownstreamFromGenerate` in `generate-session.ts`; no mount auto-run (`useStepMountAutoRun` removed from AI steps)
...
- Module: `apps/api/src/lib/ai-markdown-format/` — `formatMarkdownOnSave` helper; `formatExperienceFieldsOnSave` batches `problem` / `actions` / `outcome` into one AI call (JSON response, then per-field finalize); kinds `verdict` | `generate` | `evaluate` | `companyWhatItIs` | `experienceProblem` | `experienceActions` | `experienceOutcome`
...
- **AI POST response slimming (Phase 78)** — `POST /ai-verdict`, `/ai-resume`, `/ai-evaluate`, `/ai-experience-advise`, `/ai-combine-recommend`, and `/ai-check-gaps` return business fields + `tokenUsed` only; prompt input/output strings are persisted on `aiUsage` for History detail, not echoed in the POST response
