...
| Secrets / API keys | Per-user encrypted storage (later) | Users manage Cursor (now) and later OpenAI / Anthropic keys |
...
- Endpoints: `GET /health`, `POST /auth/register`, `POST /auth/login`, `POST /auth/logout`, `GET /auth/me`
