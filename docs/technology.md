...
- When `doVerdict` is true: Workflow shows AI Verdict result; resume generation sends that Markdown as `jobContext` instead of the raw job description (Verdict Prompt structure and extracted fields affect tailoring quality)
...
- Workflow: read-only **AI Verdict result** Markdown panel at top when `doVerdict` (`acceptedMarkdown`), with notice that it replaces the raw job description for resume generation; then one `PcewSection` workflow table (single-select); loads all workflows via `GET /workflows` with `page=null`; row select persists `lastSelectedWorkflowId`; **Next** fetches `GET /workflows/:id/generation-fingerprint`, builds `generationInputKey` (includes `jobContext` from Verdict or filtered JD + fingerprint), then runs `POST /ai-resume` or reuses stored resume when job context + workflow + PCE content are unchanged
