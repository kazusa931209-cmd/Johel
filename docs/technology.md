...
- **General Resume** (`/resume-builder`): `generations.kind` = `generalResume` (JD runs = `jdResume`); public IDs **`GEN-YYYYMMDD-###`** vs JD **`JDR-YYYYMMDD-###`**; `users.currentGeneralGenerationId` + `POST/PUT/GET /general-generations/*` (Evaluate always on; snapshots omit Job); combine JSON includes per-run `userInstruction` (not in combine defaults). AI: `POST /ai-general-combine-recommend` (`generateType: generalCombineRecommend`) uses PCE + user instruction + keyword context — not JD endpoints. Default system prompts: `DEFAULT_GENERAL_GENERATE_PROMPT` / `DEFAULT_GENERAL_EVALUATE_PROMPT` in `@johel/prompt-defaults` (snapshotted on general generation start). Client session: `useGeneralResumeSession` + `general-resume-persistence.ts`
