...
* **Combine** (per-generation snapshot, not a workspace preset) chooses profile, resume company order, which cards attach to which company, and optional run emphasis. The browser remembers the last **profile** and included **companies** (period, role context, keyword context, and linked experience ids) per signed-in user and pre-fills a **new generation**; **Run guidance** does not carry over. Each included company may have optional **Keyword context** (comma-separated steering text for experience selection); empty Keyword context means that company uses Auto (job/Verdict and role context only). **Run guidance** (`emphasis`) is separate from Keyword context and applies at Generate only. Linking uses **Suggest experiences** or manual add per company. Resume output language is configured in Settings **Generation**, not on the Combine step. Linking a card to a company asserts that work happened there.
...
* **Generate** (`/`)
  ...
  * **Combine**: choose profile, optional **Run guidance**, and **Companies & Experiences** (card grid: when none are included, sorted by company **Display Priority**; once included, selected cards follow **selection order** and unselected cards follow **Display Priority**; include toggle, period slider, horizontal **Role context** (required) and **Keyword context** (optional) label + input rows per company; linked capability cards below Keyword context with view, delete, and manual add; AI rationale under each company’s experience list after suggest; section-level warnings above cards after suggest); **Profile** and **Companies & Experiences** use bordered section panels; period labels show the month range in primary text plus a muted short inclusive duration, e.g. `Jan 2025 – Present` `(1y 9m)`; **Suggest experiences** in the section title row (beside **Reset**) maps cards per company (Keyword context filled → keyword-guided; empty → Auto); included company rows also offer **Suggest** beside **View** for that company only; include toggle is limited to the checkbox and company name; the section button label stays **Suggest experiences**; a second suggest in the same session (section-wide or per company, after links or rationale exist) opens a confirm dialog warning that linked experiences and rationale will be replaced; changing profile or included companies resets that session state; fullscreen wait overlay while suggesting; after a successful suggest in this session, a success alert below the section tells the user to click **Run** on the step bar; company selection is enabled only after a profile is selected; work period slider range is from the **profile graduation month** through the present; step-bar **Run** is disabled until profile with graduation year and month, ≥1 included company with period + role context, and at least one linked experience are ready, then advances to Generate (resume generation runs on the Generate step)
